...
class WXRequest(BaseRequest):
    ...
    def _get_token(self, url):
        err_msg = "wx request error: [%s]"
        res, code = self.req_get(url)
        if code != 200:
            if res.get('errmsg'):
                err_msg = err_msg % (res.get('errmsg').encode('gbk') or res.get('message', 'error'))
            else:
                err_msg = err_msg % (res.get('errmsg') or res.get('message', 'error'))
    ...
    def send_msg(self, data, msg_type='text', touser=None, toparty=None, totag=None, safe=0):
        ...
        支持消息类型：
            ...
            6 news消息
            7 mpnews消息
        消息内容，参考：https://qydev.weixin.qq.com/wiki/index.php?title=%E6%B6%88%E6%81%AF%E7%B1%BB%E5%9E%8B%E5%8F%8A%E6%95%B0%E6%8D%AE%E6%A0%BC%E5%BC%8F
        :return:
        """
        if (not touser) and (not toparty) and (not totag):
            raise requests.exceptions.BaseHTTPError('必须选择其中一个发送消息对象 用户/部门/标签')
        ...
        if touser:
            req_data.update(touser=touser)
        if toparty:
            req_data.update(toparty=toparty)
        if totag:
            req_data.update(totag=totag)

        content = data.get(msg_type)
        if not content:
            raise requests.exceptions.BaseHTTPError("请求参数不对，请参考微信企业号官网给出正确参数.")

        token = self.get_token()
        url = "https://qyapi.weixin.qq.com/cgi-bin/message/send?access_token=%s" % token
        req_data[msg_type] = content
        res, code = self.req_post(url, data=req_data)
        if code == 200:
            if int(res.get('errcode')) == 0:
                return True, "ok"
            elif int(res.get('errcode')) == 42001:
                # 授权码过期，重新授权再发送一次
                token = self.get_token()
                url = "https://qyapi.weixin.qq.com/cgi-bin/message/send?access_token=%s" % token
                res, code = self.req_post(url, data=req_data)
                ...
                logger.error(res)
                return False, res.get('errmsg')
        return False, "请求服务错误[%s]" % code

    def upload_file(self, media, file_type='file'):
        """
        微信临时上传素材接口
        https://qydev.weixin.qq.com/wiki/index.php?title=%E4%B8%8A%E4%BC%A0%E4%B8%B4%E6%97%B6%E7%B4%A0%E6%9D%90%E6%96%87%E4%BB%B6
        :param media: 文件内容数据为字典
        :param file_type: 素材类型，包括普通文件file,图片image,视频vedio等，具体类型参考微信企业号官网接口
        :return:
        """
        if not isinstance(media, dict):
            raise requests.exceptions.BaseHTTPError("media必须是字典")
        if not media.get('media'):
            raise requests.exceptions.BaseHTTPError("请求参数不对，参数字典中必须包含'media'.")

        token = self.get_token()
        url = "https://qyapi.weixin.qq.com/cgi-bin/media/upload?access_token=%s&type=%s" % (token, file_type)
        res, code = self.req_post(url, files=media)
        if code == 200:
            if int(res.get('errcode', '0')) == 0:
                return True, res
            else:
                return False, res.get('errmsg')
        return False, "请求服务错误[%s]" % code
    ...
    def download_file(self, media_id):
        """
        下载素材
        https://qydev.weixin.qq.com/wiki/index.php?title=%E8%8E%B7%E5%8F%96%E4%B8%B4%E6%97%B6%E7%B4%A0%E6%9D%90%E6%96%87%E4%BB%B6
        :param media_id:
        :return:
        """
        url = "https://qyapi.weixin.qq.com/cgi-bin/media/get?access_token=%s&media_id=%s" % (self.get_token(), media_id)
        ...
        if res.status_code == 200:
            try:
                ...
                return False, content.get('errmsg')
            except:
                data = {
